...
export default function CheckoutPage() {
  ...
  // Order history check (for shipping fee logic)
  const [hasOldOrders, setHasOldOrders] = useState(false);
  ...
  // ─── Shipping fee logic ──────
  const EXPRESS_EXTRA = 20;
  const STANDARD_RETURNING = 30;
  const EXPRESS_CHARGE = 50; 

  const getShippingCharge = () => {
    if (!isAuthenticated) {
      return deliveryType === 'express' ? EXPRESS_CHARGE : 0;
    }
    // authenticated
    if (hasOldOrders) {
      return deliveryType === 'express'
        ? STANDARD_RETURNING + EXPRESS_EXTRA  
        : STANDARD_RETURNING;                 
    } else {
      return deliveryType === 'express' ? EXPRESS_CHARGE : 0; 
    }
  ...
  const checkAuthentication = async () => {
    try {
      ...
      if (token && customer && customer.name) {
        ...
        await Promise.all([fetchAddresses(), checkOrderHistory()]);
      } else {
  ...
  const checkOrderHistory = async () => {
    try {
      ...
      setHasOldOrders(nonCancelledOrders.length > 0);
    } catch {
      setHasOldOrders(false);
    }
  };
  ...
  const submitOrder = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        clearCart();
        router.push(
          `/order-success?orderId=${response.data?.id || 'N/A'}&total=${totalAmount}&payment=${paymentMethod}&delivery=${deliveryType}`
        );
      }
    } catch (error) {
      ...
      <div className="bg-gray-50 min-h-screen flex flex-col pb-20 md:pb-8">
        ...
        <div className="bg-white shadow-sm sticky top-0 z-40">
          <div className="max-w-7xl mx-auto px-4 py-3">
            <div className="flex items-center text-gray-800 gap-4">
              <Link
                href="/"
                className="p-2 hover:bg-gray-100 text-gray-800 rounded-full transition cursor-pointer"
              >
                <ChevronLeft size={24} />
              </Link>
        ...
        <div className="max-w-4xl mx-auto w-full px-4 py-6">
          ...
          {deliveryTimeInfo.show && (
            <div
              className={`${deliveryTimeInfo.bg} border ${deliveryTimeInfo.border} rounded-lg px-4 py-3 mb-6 flex items-center gap-3`}
            >
              <svg
                className="w-5 h-5 flex-shrink-0 text-current text-gray-800"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
          ...
          <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              >
                ...
                <p
                  className={`text-xs text-center mt-1 ${
                    deliveryType === 'standard' ? 'text-white/90' : 'text-gray-500'
                  }`}
                >
                  Within 3 hours
                </p>
                <p
                  className={`text-xs text-center mt-0.5 font-medium ${
                    deliveryType === 'standard' ? 'text-white' : 'text-[#FF5533]'
                  }`}
                >
                  {isAuthenticated && hasOldOrders ? '৳30' : 'Free'}
                </p>
              </button>
              ...
              >
                ...
                <p
                  className={`text-xs text-center mt-1 ${
                    deliveryType === 'express' ? 'text-white/90' : 'text-gray-500'
                  }`}
                >
                  Within 1 hour
                </p>
                <p
                  className={`text-xs text-center mt-0.5 font-medium ${
                    deliveryType === 'express' ? 'text-white' : 'text-[#FF5533]'
                  }`}
                >
                  {isAuthenticated && hasOldOrders ? '৳50' : '৳50'}
                </p>
              </button>
            </div>
          </div>

          {/* Order Note (authenticated only) */}
          {isAuthenticated && (
            <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
